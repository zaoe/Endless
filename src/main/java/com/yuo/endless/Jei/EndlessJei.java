...
public class EndlessJei implements IModPlugin {
    ...
    public void registerCategories(IRecipeCategoryRegistration registration) {
        registration.addRecipeCategories(new ExtremeCraftRecipeCategory(registration.getJeiHelpers().getGuiHelper()),
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        registration.addRecipes(recipeManager.getRecipesForType(EndlessRecipeType.EXTREME_CRAFT).stream()
                .filter(r -> r instanceof ExtremeCraftRecipe).collect(Collectors.toList()), ExtremeCraftRecipeCategory.UID);
        registration.addRecipes(recipeManager.getRecipesForType(EndlessRecipeType.NEUTRONIUM).stream()
    ...
    public void registerRecipeTransferHandlers(IRecipeTransferRegistration registration) {
        registration.addRecipeTransferHandler(ExtremeCraftContainer.class, ExtremeCraftRecipeCategory.UID, 0, 80, 81, 36);
    }
}
